...
recovery_times <- read_csv(here("data/recovery_times.csv"))
late_deliveries <- read_csv(here("data/late_deliveries.csv"))
fitness_levels <- read_csv(here("data/fitness_levels.csv"))
```
...
ggplot(recovery_times) +
  aes(recovery, treatment_group, fill = prognosis) +
  geom_col()
```
...
ggplot(fitness_levels) +
  aes(age, fitness_score, fill = group ) +
  geom_col() +
  facet_wrap (~child, nrow = 5)

#I kind of like the visualization of facet_grid better, but this obviously looks cleaner. If I had more time, I would  probably scale y.
```
...
* `car_use`  
```{r}
blood_pressure <- read_csv(here("data/blood_pressure.csv"))
names(blood_pressure)
```
```{r}

ggplot(blood_pressure) +
  aes(daily_saturated_fat_intake_mg, systolic_blood_pressure_mm_hg) +
  geom_line()
```


```{r}
car_use <- read_csv(here("data/car_use.csv"))
```
```{r}
...
#Cameron code
ggplot(car_use) +
  aes(x = population, y = air_so2_concentration_ppm, colour = car_use_percent, size = car_use_percent) +
  geom_point() +
  scale_colour_gradient(low="#d1b8ff", high="#1a0269")
```
```{r}
#John code
car_use <- CodeClanData::car_use
ggplot(car_use, mapping = aes(x = population, y = air_so2_concentration_ppm, colour = car_use_percent)) +
  geom_point(size = 2) +
...
car_use %>% 
  ...
  geom_point(aes(fill = population, size = population), shape = 22)+
...
```{r}
#David code
CodeClanData::car_use %>% 
  ggplot() +
  aes(x = population, y = air_so2_concentration_ppm,
      colour = car_use_percent, size = car_use_percent) +
  geom_point() +
