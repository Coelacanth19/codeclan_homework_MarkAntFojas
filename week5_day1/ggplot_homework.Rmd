...
<br>
```{r}
time <- (qb_device_data$hours) + (qb_device_data$mins/60)
...
qb_device_data %>%                             
  ...
  ggplot()+
    ...
    geom_line(aes(x = time, y = counts, group = device, colour = device), alpha = .25) +
  geom_point(aes(x = time, y = counts, colour = device), alpha = 0.4) +
  geom_smooth(aes(x = time, y = counts, colour = device)) +
