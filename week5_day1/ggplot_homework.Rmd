...
<br>
```{r}
time <- (as.numeric(qb_device_data$hours) + as.numeric(qb_device_data$mins/60)
  
qb_device_data %>%                             
  group_by(device, id) %>% 
  arrange(hours, mins) %>% 
  ggplot()+
    aes(time, counts, group = id, colour = device) + 
    geom_line(aes(x = time, y = counts, group = chick, colour = device), alpha = .25) +
  geom_point(aes(x = time, y = counts, colour = device), alpha = 0.4) +
  geom_smooth(aes(x = time, y = counts, colour = devic)) +
