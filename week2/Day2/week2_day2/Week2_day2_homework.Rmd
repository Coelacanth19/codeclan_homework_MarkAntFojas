...
```{r}
str_detect(whole_text, "Thought\ Police")
str_count(whole_text, "Thought\ Police")
...
```{r}
str_count(whole_text, "Big\ Brother")
```
...
```{r}
#ugly
pattern <- "[Jan|Feb|Mar|Apr|Jul|Aug|Sep|Nov|Dec]+\ [0-9]th,\ [0-9]+"
str_extract_all(whole_text, pattern)

#attempting date.as statement again
pattern <- date = date.as
```
...
```{r}
str_replace_all(whole_text, "Winston", replacement = "REDACTED")
```
...
```{r}
str_replace(whole_text, "VICTORY\ GIN", replacement = "VICTORY VODKA")
```
...
```{r}
pattern <- "Mini"
str_extract_all(whole_text, "Mini[true|luv+|pax+|plenty+]")

```
```{r}
str_replace_all(whole_text, "Ministry+", replacement = "Department")
str_replace_all(whole_text, "Minitrue+", replacement = "Deptrue")
str_replace_all(whole_text, "Minipax+", replacement = "Deppax")
str_replace_all(whole_text, "Miniluv+", replacement = "Depluv")
str_replace_all(whole_text, "Miniplenty+", replacement = "Depplenty")
```
